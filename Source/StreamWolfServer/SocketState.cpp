...
namespace StreamWolf {
    namespace Net {
        namespace Sockets {
            ...
            {
                IPEndPointPtr point;
                int32_t family = (int32_t)socket->Family();
                int32_t type = (int32_t)socket->Type();
                int32_t protocol = (int32_t)socket->Protocol();
            ...
            void Socket::SocketState::Listen(Socket* socket, uint32_t backlog)
            {
                throw socket_error("Socket is not bound to an end point");
            }

            int32_t Socket::SocketState::Receive(Socket* socket, std::vector<uint8_t>& buffer, uint32_t offset, uint32_t size, SocketFlags socketFlags, SocketError& errorCode)
            {
            ...
            void Socket::SocketBound::Listen(Socket* socket, uint32_t backlog)
            {
            ...
            int32_t Socket::SocketConnected::Receive(Socket* socket, std::vector<uint8_t>& buffer, uint32_t offset, uint32_t size, SocketFlags socketFlags, SocketError& errorCode)
            {
                if (offset > buffer.size() || size > buffer.size() - offset) {
                    throw std::out_of_range("offset and size does not match buffer size");
                }
